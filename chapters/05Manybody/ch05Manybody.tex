...
\subsubsection{The general shape}
...
where $\beta$ is a variational parameter, and $a_{ij}$ is a spin-dependent constant tuned to obey the cusp condition from Section \ref{sec:correlations}.
...
In the case of well-behaving systems, a single determinant converges rapidly enough. This simplicity opens up the possibility of simulating large systems efficiently.
...
variance blah blah, blocking blah blah
...
Not having to normalize our wave functions saves us a lot of CPU-time, but it also relieves us of complicated normalization factors in our single particle basis expressions. Each of these single particle normalization factors serve as a constant multiplying a row in the Slater determinant. From mathematics we know that multiplying a row in a determinant with a constant is the same as multiplying the determinant by the constant, hence we can skip all normalization constants in all the basis expressions as well.


\subsection{The Variational Principle}

The $K=1$ single-determinant wave function is a very simple approximation. In order to optimize the overlap with the exact wave function, variational parameters are introduced in the trial wave function. 
Looking at Eq.~(\ref{eq:jastrow}), we already have $\beta$. Another variational parameter, $\alpha$, is introduced in the spatial part: 
...
\begin{eqnarray*}
 ...
 E   &=& \bra{\Psi_T(\alpha, \beta)}\OP{H}\ket{\Psi_T(\alpha, \beta)}\\
     ...
     &=& \sum_k |C_k|^2E_k% = \sum_k |C_k^2| (E_0 + \delta E_k)
\end{eqnarray*}
...
\subsection{FINNE OPTINMALE PARAMMETERS LOL}
