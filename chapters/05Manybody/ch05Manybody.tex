...
\label{FIG:BlockingExamples}
 \begin{center}
  ...
  \caption{Left hand side: Blocking result of (approximately) uncorrelated data generated from a uniform Monte-Carlo integration of $\int _1^2 2x\mathrm{d}x$ resulting in $3.00003$ (exact is $3.0$). This is in excellent agreement with the magnitude of the error $\sim 9\cdot 10^{-5}$. On the right we have the \textit{VMC} result (see Section \ref{sec:VMC}) of a 12-particle Quantum Dot system. We clearly see the plateau discussed in the blocking section, resulting in a total error of $\sim 6\cdot 10^{-4}$.}
 \end{center}
\end{figure}
