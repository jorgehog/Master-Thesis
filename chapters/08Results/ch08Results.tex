...
\begin{landscape}
 ...
 \begin{tabular}{rl}
  ...
  \rot{$\qquad\quad\omega=0.01$}&\subfigure[$N=2$]{\includegraphics[scale=\OBDscale]{../Graphics/OBD/OBD_DMC/dist_out_QDots2c01_3D.png}}
 ...
 \end{tabular}
  \caption{\small{Quantum Dots one-body densities. Each row represents a given value of the oscillator frequency (see the side row labels), and each column represents a given particle number (see the bottom labels). Notice that the densities for $\omega=1$ from figure \ref{fig:OBD_DMC_QDOTS_w1} are indistinguishable from those of $\omega=0.28$ except for their radial extent. This trend has been verified in the case of $N=30$, 42 and 56 electrons as well as for omega=0.5, however, for the sake of transparency, these results are left out of the current figure. Results for $N=30$, 42 and 56 particles for $\omega=0.01$ has too wide radial extent to converge with the computational resources available at the present time and has thus not been computed.}}
